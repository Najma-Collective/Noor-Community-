...
function sanitizeHTML(html, { allowedTags = DEFAULT_ALLOWED_TAGS, allowedAttributes = DEFAULT_ALLOWED_ATTRIBUTES } = {}) {
...
export function createEditorCore({
  canvas,
  placeholder = canvas?.querySelector(".canvas__placeholder") ?? null,
  ...
  if (!(canvas instanceof HTMLElement)) {
    throw new Error("A valid canvas element is required to initialise the editor core.");
  }
  ...
  function setActiveBlock(block) {
    ...
    highestZIndex += 1;
    block.style.zIndex = String(highestZIndex);
  }
  ...
  function serializeBlocks() {
    const blocks = Array.from(canvas.querySelectorAll(".block")).map((block) => {
      ...
      if (type === "text") {
        ...
        const moduleContent = block.querySelector(".module-content");
        const header = block.querySelector(".block__body > header");
  ...
  function persistState() {
    ...
    if (typeof onStateChange === "function") {
      try {
        ...
        console.error("onStateChange callback failed", error);
      }
    }
  ...
  function clearBlocks() {
    canvas.querySelectorAll(".block").forEach((block) => block.remove());
    updatePlaceholderVisibility();
    highestZIndex = 1;
  }

  function loadState(state, { replaceExisting = true, skipPersist = false } = {}) {
    if (replaceExisting) {
      clearBlocks();
    }
    ...
    state.blocks.forEach((data) => {
      ...
      if (typeof data.zIndex === "number") {
        block.style.zIndex = String(data.zIndex);
  ...
  function restoreStateFromStorage({ replaceExisting = true } = {}) {
    if (!storageKey || !window.localStorage) {
      updatePlaceholderVisibility();
      return null;
    }
    ...
    try {
      ...
      loadState(state, { replaceExisting, skipPersist: true });
      return state;
    } catch (error) {
  ...
  function registerBlockInteractions(interactLib = interactInstance ?? globalThis.interact) {
    if (!interactLib) {
      console.warn("Interact.js is required to enable drag and resize interactions.");
      return null;
    }

    interactLib(".block").draggable({
      listeners: {
        start(event) {
    ...
    interactLib(".block").resizable({
      ...
      modifiers: [
        interactLib.modifiers.restrictEdges({
          outer: "parent",
        }),
        interactLib.modifiers.restrictSize({
